...
raw_niche_data_mess <- read_csv("/Users/marco/GitHub/environmental_breadth_final/3_generated_data/raw_niche_data_mess.csv")

# -------------------------------------------------------------------------------------------------------
##### Reading in the species occurrences
occurrences_trees <- read_csv(paste("/Users/marco/GitHub/environmental_breadth_final/1_original_data/tree_occurrences_Andrea_clean_withElevation.csv", sep = "")) %>%
...
occurrences_herbs <- read_csv(paste("/Users/marco/GitHub/environmental_breadth_final/1_original_data/herb_occs_elevation-105450sp.csv", sep = "")) %>%
  filter(Species %in% raw_niche_data_mess$Species) %>%
  mutate(hemisphere = sign(latitude)) %>%
  filter(hemisphere != 0) %>%
  mutate(growthform = "herb") %>%
...
latitudinal_stats_g <- occurrences %>% 
  group_by(Species) %>% 
  filter(sum(!is.na(latitude)) >= 5) %>% # Keep groups with at least 5 non-NA latitude values
  summarize(lat_range_sd_g = round(sd(abs(latitude), na.rm = TRUE), digits = 3), 
            lat_range_mad_g = round(mad(abs(latitude), na.rm = TRUE), digits = 3),
            lat_median_g = round(sign(median(latitude, na.rm = TRUE))*median(abs(latitude), na.rm = TRUE), digits = 3))

latitudinal_stats_n <- occurrences %>% 
  filter(hemisphere == 1) %>%
  group_by(Species) %>% 
  filter(sum(!is.na(latitude)) >= 5) %>% # Keep groups with at least 5 non-NA latitude values
  summarize(lat_range_sd_n = round(sd(abs(latitude), na.rm = TRUE), digits = 3), 
            lat_range_mad_n = round(mad(abs(latitude), na.rm = TRUE), digits = 3),
            lat_median_n = round(median(abs(latitude), na.rm = TRUE), digits = 3))

latitudinal_stats_s <- occurrences %>% 
  filter(hemisphere == -1) %>%
  group_by(Species) %>% 
  filter(sum(!is.na(latitude)) >= 5) %>% # Keep groups with at least 5 non-NA latitude values
  summarize(lat_range_sd_s = round(sd(abs(latitude), na.rm = TRUE), digits = 3), 
            lat_range_mad_s = round(mad(abs(latitude), na.rm = TRUE), digits = 3),
            lat_median_s = round(median(abs(latitude), na.rm = TRUE), digits = 3))
...
latitudinal_stats <- latitudinal_stats_g %>%
  ...
  select("Species", "lat_range_sd_n", "lat_range_mad_n", "lat_median_n", "lat_range_sd_s", "lat_range_mad_s", "lat_median_s", "lat_range_sd_g", "lat_range_mad_g", "lat_median_g")
...
# Export
write_csv(niche_data, "/Users/marco/GitHub/environmental_breadth_final/3_generated_data/niche_data_final_summarized_v5_withMAD_min5_sign.csv")
