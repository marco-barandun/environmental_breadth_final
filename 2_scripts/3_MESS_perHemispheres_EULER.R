...
run_mess <- function(species_list, envs_var) {
  ...
  for (species in species_list) {
    ...
    model <- readRDS(paste("./res_saved_models/", species, "_models.rds", sep = ""))
    ...
    write_csv(data.frame(species, value_N, value_S), paste("./res_MESS_perHemisphere/", species, "_MESS_perHemisphere.csv", sep = ""))
    
    writeRaster(ms_N, filename= paste("./res_MESS_perHemisphere_tif/", species, "_MESS_perHemisphere_N.tif", sep = ""), format="GTiff")
    writeRaster(ms_S, filename= paste("./res_MESS_perHemisphere_tif/", species, "_MESS_perHemisphere_S.tif", sep = ""), format="GTiff")
...
envs <- raster::stack(list.files(path="./envs_standardized/", pattern='*.tif$*', full.names=TRUE)) %>%
  raster::brick(.)

# view the subsetted raster stack
plot(envs %>% crop(., extent(-180, 180, -90, 0)))
  
modelled_species <- list.files(path ="./res_saved_models/") %>%
  sub("_models.rds", "", .)
MESS_perHemisphere_species <- list.files(path ="./res_MESS_perHemisphere/") %>%
  sub("_MESS_perHemisphere.csv", "", .)

species_to_run <- modelled_species %>%
  setdiff(., MESS_perHemisphere_species)

parallel::mclapply(head(species_to_run), function(species) {
